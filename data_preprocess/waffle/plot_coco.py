...
def visualize_coco_polygons(
    coco_json_path, image_path=None, save_path="visualization.png", show_bbox=True, show_labels=True
):
    """
    ...
    for idx, annotation in enumerate(coco_data["annotations"]):
        ...
        if "segmentation" in annotation:
            for seg in annotation["segmentation"]:
                ...
                poly = Polygon(points, facecolor=color + (0.3,), edgecolor=color, linewidth=2)
                ax.add_patch(poly)
        ...
        if show_bbox and "bbox" in annotation:
            ...
            rect = patches.Rectangle(
                (bbox[0], bbox[1]), bbox[2], bbox[3], linewidth=1, edgecolor=color, facecolor="none", linestyle="--"
            )
            ax.add_patch(rect)
...
def visualize_single_annotation(coco_json_path, annotation_id, save_path=None):
    ...
    if "segmentation" in annotation:
        for seg in annotation["segmentation"]:
            ...
            # Plot filled polygon
            poly = Polygon(points, facecolor="blue", alpha=0.3, edgecolor="blue", linewidth=3)
            ax.add_patch(poly)
    ...
    if "bbox" in annotation:
        bbox = annotation["bbox"]
        rect = patches.Rectangle(
            (bbox[0], bbox[1]), bbox[2], bbox[3], linewidth=2, edgecolor="red", facecolor="none", linestyle="--"
        )
        ax.add_patch(rect)
...
def create_overlay_visualization(coco_json_path, output_path="coco_overlay.png"):
    ...
    for idx, annotation in enumerate(coco_data["annotations"]):
        ...
        if "bbox" in annotation:
            ...
            cv2.putText(canvas, label, (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)

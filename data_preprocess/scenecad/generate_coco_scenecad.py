...
def config():
    ...
    a.add_argument(
        "--train_list",
        default="scenecad/meta_data/scannetv2_train.txt",
        type=str,
        help="path to official train scene list",
    )
    a.add_argument(
        "--val_list", default="scenecad/meta_data/scannetv2_val.txt", type=str, help="path to official val scene list"
    )
            ...
            for i, corner in enumerate(polys):
                if i == len(polys) - 1:
                    cv2.line(
                        ...
                        (round(polys[0][0]), round(polys[0][1])),
                        (0, 252, 252),
                        1,
                    )
                else:
                    cv2.line(
                        density_img_vis,
                        (round(corner[0]), round(corner[1])),
                        (round(polys[i + 1][0]), round(polys[i + 1][1])),
                        (0, 252, 252),
                        1,
                    )

...
WANDB_MODE=online python -m torch.distributed.run --nproc_per_node=1 --master_port=$MASTER_PORT main_ddp.py --dataset_name=stru3d \
               ...
               --num_polys=30 \
               --semantic_classes=-1 \
               --job_name=s3d_bw_ddp_poly2seq_l${SEQ_LEN}_nosem_bs32_coo${COO_COEFF}_cls${CLS_COEFF}_nopolyrefine_predecPE_deccatsrc_t2 \
               --batch_size 32 \
               --input_channels=3 \
               ...
               --ema4eval \
               # --dec_layer_type='v6' \
               # --resume output/s3d_bw_ddp_poly2seq_l${SEQ_LEN}_nosem_bs32_coo${COO_COEFF}_cls${CLS_COEFF}_nopolyrefine_predecPE_decv41_t1/checkpoint.pth
               # --clip_max_norm 1.0 \
               # --debug
               # --dec_qkv_proj \
            #    --set_cost_coords=10 \

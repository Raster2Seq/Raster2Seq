...
NUM_BINS=32
CONVERTER=v3
DATA=data/coco_waffle_finetune_dataset/
JOB=waffle_l${SEQ_LEN}_bin${NUM_BINS}_sem${SEM_COEFF}_coo${COO_COEFF}_cls${CLS_COEFF}_anchor_deccatsrc_smoothing_cls12_convert${CONVERTER}_fromcubi_t5
...
WANDB_MODE=online torchrun --nproc_per_node=1 --master_port=$MASTER_PORT main_ddp.py --dataset_name=waffle \
               --dataset_root=${DATA} \
               --num_queries=2800 \
               --num_polys=50 \
               --semantic_classes=2 \

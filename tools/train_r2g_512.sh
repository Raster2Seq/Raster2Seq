...
WANDB_MODE=online python -m torch.distributed.run --nproc_per_node=${NUM_GPUS} --master_port=$MASTER_PORT  main_ddp.py --dataset_name=r2g \
               ...
               --num_polys=50 \
               --semantic_classes=13 \
               --job_name=r2g_res512_queries56x50_fromckpt_sem13 \
               --batch_size 4 \
               --input_channels=3 \
...
# NUM_BINS=32
# CONVERTER=v3
# JOB=r2g_res512_poly2seq_l${SEQ_LEN}_bin${NUM_BINS}_sem${SEM_COEFF}_coo${COO_COEFF}_cls${CLS_COEFF}_anchor_deccatsrc_smoothing_cls13_convert${CONVERTER}_frompretrainckpt_t1

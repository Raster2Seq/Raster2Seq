...
def enhance_floorplan(img_arr):
    ...
    denoised = cv2.fastNlMeansDenoisingColored(
        img_arr, None, h=5, hColor=5, templateWindowSize=7, searchWindowSize=21
    )
...
def setup_finetune_dataset(val_split=0.1, test_split=0.1):
    ...
    for split in ['train', 'val', 'test']:
        output = {
            ...
            "annotations": [x for x in datasets[split]['annotations'] if x['image_id'] in image_id_having_annotations[split]],
...
def run_finetuning(dataset_dir, num_classes):
    ...
    base_ckpt_path = Path("output/cubi_v4-1refined_poly2seq_l512_bin32_sem1_coo20_cls5_anchor_deccatsrc_smoothing_cls12_convertv3_fromnorder499_t1/checkpoint0499.pth") #  base_dir / 'checkpoints' / 'checkpoint.pth'

...
class DeformableTransformer(nn.Module):
    def __init__(
        ...
        if dec_layer_type == "v1":
            ...
            decoder_layer = TransformerDecoderLayerV2(
                d_model,
                dim_feedforward,
                dropout,
                activation,
            ...
            decoder_layer = TransformerDecoderLayerV4(
                d_model,
                dim_feedforward,
                dropout,
                activation,
            ...
            decoder_layer = TransformerDecoderLayerV41(
                d_model,
                dim_feedforward,
                dropout,
                activation,
            ...
            decoder_layer = TransformerDecoderLayerV5(
                d_model,
                dim_feedforward,
                dropout,
                activation,
            ...
            decoder_layer = TransformerDecoderLayerV6(
                d_model,
                dim_feedforward,
                dropout,
                activation,
    ...
    def forward(
        ...
        if attn_concat_src:
            ...
            for i, (H_, W_) in enumerate(src_spatial_shapes):
                pool_src = (
                    F.adaptive_avg_pool2d(rearrange(src_list[i], "b (h w) d -> b d h w", h=H_), (1, 1))
                    .flatten(2)
                    .transpose(1, 2)
                )
            ...
            tgt_masks = torch.cat([torch.zeros(q.size(1), pool_src.size(1), device=q.device), tgt_masks], dim=1).to(
                dtype=torch.float32
            )
...
class TransformerDecoderLayerV4(nn.Module):
    ...
    def _reset_parameters(self):
        ...
        constant_(self.source_proj.bias.data, 0.0)
    ...
    def _sample_reference_points(self, query, src, src_spatial_shapes, level_start_index):
        ...
        sampling_offsets = self.sampling_offsets(query).view(
            query.size(0), query.size(1), self.n_heads, self.n_levels, self.n_points, 2
        )
        ...
        attention_weights = self.attention_weights(query).view(
            query.size(0), query.size(1), self.n_heads, self.n_levels * self.n_points
        )
    ...
    def forward(
        ...
        if attn_concat_src:
            ...
            tgt_masks = torch.cat([torch.zeros(q.size(1), sampled_src.size(1), device=q.device), tgt_masks], dim=1)
...
class TransformerDecoderLayerV2(nn.Module):
    def __init__(
        self, d_model=256, d_ffn=1024, dropout=0.1, activation="relu", n_levels=4, n_heads=8, n_points=4, **kwargs
    ):
        super().__init__()
        self.d_model = d_model
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not is_last_layer:
            ...
            self.cross_attn = CAOneSidedBlock(
                d_model,
                d_model,
                d_model,
                n_heads,

"""
This is a hack implementation for evaluation on RPlan
...
class Evaluator_RPlan:
    ...
    def evaluate_scene(
        ...
        gt_window_doors_list=None,
        window_door_lines_types=None,
        gt_window_doors_type_list=None,
        show=False,
        name="ours",
        dataset_type="s3d",
        img_size=(256, 256),
    ...
    def get_quantitative(
        ...
        if pred_window_doors_types is not None:
            if self.wd_as_line:
                ...
                for wd_ind, poly in enumerate(gt_window_doors):
                    wd_map = np.zeros((h, w))
                    cv2.fillPoly(wd_map, [poly], color=1.0)
                    gt_wd_map_list.append(wd_map)

                gt_wd_polys_sorted_indcs = [
                    i[0] for i in sorted(enumerate(gt_wd_map_list), key=poly_map_sort_key, reverse=True)
                ]
        ...
        room_metric_rec = sum(room_metric) / float(len(gt_polys) + 1e-4)
        ...
        if pred_types is not None:
            ...
            room_sem_metric_rec = sum(room_sem_metric) / float(len(gt_polys) + 1e-4)
        ...
        if pred_window_doors_types is not None:
            ...
            window_door_metric_rec = sum(pred2gt_exists_wd) / float(len(gt_window_doors) + 1e-4)
        ...
        corner_metric_rec = sum(corner_metric) / float(gt_corners_n + 1e-4)
        ...
        angles_metric_rec = sum(angles_metric) / float(gt_corners_n + 1e-4)

    ...
    if config.test_dataset == "pascal":
        ...
        for split in splits:
            ...
            if shift == "auto":
                ...
                shift = compute_shift2(
                    model,
                    [get_cached_pascal_pfe(s, config) for s in range(4) if s != split],
                    repetitions=config.compute_shift_fac,
                )
                log.info(
                    f"Best threshold is {shift}, computed on splits: {[s for s in range(4) if s != split]}, took {time.time() - shift_compute_t:.1f}s"
                )
            ...
            assert (
                config.batch_size is None or config.batch_size == 1
            ), "When PFE Dataset is used, batch size must be 1"
        ...
        valid_keys = [
            k
            for k in all_keys
            if all(v[k] is not None and isinstance(v[k], (int, float, np.float)) for v in scores.values())
        ]
